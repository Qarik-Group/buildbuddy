...
func (ar *actionRunner) Run(ctx context.Context, ws *workspace) error {
	...
	for i, bazelCmd := range ar.action.BazelCommands {
		...
		if exitCode == 0 && runScript != "" {
			runInfo, err := processRunScript(ctx, ws, runScript)
			if err != nil {
				return err
			}
			e := &bespb.BuildEvent{
...
func uploadRunfiles(ctx context.Context, workspaceRoot, runfilesDir string) ([]*bespb.File, error) {
	digestMap := make(map[digest.ResourceName]string)
	...
	for d, runfilePath := range digestMap {
		f, err := os.Open(runfilePath)
		if err != nil {
			return nil, err
		}
		if err := u.Upload(d.GetDigest(), f); err != nil {
			return nil, err
		}
		...
		runFiles = append(runFiles, &bespb.File{
			Name: relPath,
			File: &bespb.File_Uri{
				Uri: fmt.Sprintf("bytestream://remote.buildbuddy.io/%s", d.DownloadString()),
			},
		})
	}
...
func processRunScript(ctx context.Context, ws *workspace, runScript string) (*runInfo, error) {
